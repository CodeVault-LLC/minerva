...
func StripeAccessToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	// validate
	tps := []string{
		"stripeToken := \"sk_test_" + utils.NewSecret(alphaNumeric("30")) + "\"",
	...
	fps := []string{"nonMatchingToken := \"task_test_" + utils.NewSecret(alphaNumeric("30")) + "\""}
