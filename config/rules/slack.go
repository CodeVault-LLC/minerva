...
func SlackBotToken() *types.Rule {
	...
	tps := []string{
		...
		`"bot_token4": ` + fmt.Sprintf(`"xoxb-%s-%s-%s"`, utils.NewSecret(numeric("13")), utils.NewSecret(numeric("12")), utils.NewSecret(alphaNumeric("24"))),
	}
	fps := []string{
...
func SlackUserToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	tps := []string{
		...
		`"user_token6": ` + fmt.Sprintf(`"xoxp-%s-%s-%s-%s"`, utils.NewSecret(numeric("12")), utils.NewSecret(numeric("13")), utils.NewSecret(numeric("13")), utils.NewSecret(alphaNumeric("32"))),
...
func SlackAppLevelToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	tps := []string{
		...
		`"token4": ` + fmt.Sprintf(`"xapp-1-A%s-%s-%s"`, utils.NewSecret(numeric("10")), utils.NewSecret(numeric("13")), utils.NewSecret(alphaNumeric("64"))),
	}
	return validate(r, tps, nil)
}

// Reference: https://api.slack.com/authentication/config-tokens
func SlackConfigurationToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	tps := []string{
		...
		`"access_token3": "xoxe.xoxp-1-` + utils.NewSecret(alphaNumeric("163")) + `"`,
		...
		`"access_token5": "xoxe.xoxb-1-` + utils.NewSecret(alphaNumeric("165")) + `"`,
	}
	fps := []string{
...
// Reference: https://api.slack.com/authentication/config-tokens
func SlackConfigurationRefreshToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	tps := []string{
		...
		`"refresh_token3": "xoxe-1-` + utils.NewSecret(alphaNumeric("146")) + `"`,
	}
...
func SlackLegacyBotToken() *types.Rule {
	r := types.Rule{
		...
		// This types.Rule is based off the limited information I could find and may not be 100% accurate.
		Regex: regexp.MustCompile(
	...
	tps := []string{
		...
		`"bot_token10": ` + fmt.Sprintf(`"xoxb-%s-%s`, utils.NewSecret(numeric("10")), utils.NewSecret(alphaNumeric("24"))), // gitleaks:allow
		`"bot_token11": ` + fmt.Sprintf(`"xoxb-%s-%s`, utils.NewSecret(numeric("12")), utils.NewSecret(alphaNumeric("23"))), // gitleaks:allow
	}
	fps := []string{
...
func SlackLegacyWorkspaceToken() *types.Rule {
	r := types.Rule{
	...
	tps := []string{
		...
		`"access_token1": ` + fmt.Sprintf(`"xoxa-%s-%s`, utils.NewSecret(numeric("1")), utils.NewSecret(alphaNumeric("12"))),
		`"access_token2": ` + fmt.Sprintf(`"xoxa-%s`, utils.NewSecret(alphaNumeric("12"))),
		`"refresh_token1": ` + fmt.Sprintf(`"xoxr-%s-%s`, utils.NewSecret(numeric("1")), utils.NewSecret(alphaNumeric("12"))),
		`"refresh_token2": ` + fmt.Sprintf(`"xoxr-%s`, utils.NewSecret(alphaNumeric("12"))),
	}
	fps := []string{
		// "xoxa-faketoken",
...
func SlackLegacyToken() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	tps := []string{
		...
		`"access_token6": "xoxs-` + fmt.Sprintf("%s-%s-%s-%s", utils.NewSecret(numeric("10")), utils.NewSecret(numeric("10")), utils.NewSecret(numeric("10")), utils.NewSecret(hex("10"))) + `"`,
...
func SlackWebHookUrl() *types.Rule {
	// define types.Rule
	r := types.Rule{
	...
	// validate
	tps := []string{
		"hooks.slack.com/services/" + utils.NewSecret(alphaNumeric("44")),
		"http://hooks.slack.com/services/" + utils.NewSecret(alphaNumeric("45")),
		"https://hooks.slack.com/services/" + utils.NewSecret(alphaNumeric("46")),
		...
		"hooks.slack.com/workflows/" + utils.NewSecret(alphaNumeric("44")),
		"http://hooks.slack.com/workflows/" + utils.NewSecret(alphaNumeric("45")),
		"https://hooks.slack.com/workflows/" + utils.NewSecret(alphaNumeric("46")),
